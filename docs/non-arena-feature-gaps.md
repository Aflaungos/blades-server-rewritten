...
| **Town vendor shops** | `POST /shops/{id}`(+`/sell`,`/purchase`,`/auth/refreshloot`) | ✅ Done | A merchant is now a persistent trading partner: a **10-hour window** (measured; retail's real cycle, from first visit, not wall-clock aligned) persisted in `server_state.shops`, carrying a **static gold budget** drawn from the per-`(buildingTypeId, level)` band measured off retail `catalog.wallet`, a signed `revenue` ledger, finite stock, and 5-minute buybacks. Selling pays APK `sellValue × temperMult + enchant values`, clamped to the merchant's remaining budget — and a drained merchant still takes the item and pays 0, as retail did. All 94 town-vendor bundles are priceable (was 5). Fixes tracker #30's "merchants don't have money". |
